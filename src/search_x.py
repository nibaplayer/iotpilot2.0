...
def build_workflow_with_config(workflow_config):
    ...
    print("Workflow:", execution_order)
...
def run_workflow(workflow, code_name, problem, run_iters, iter_num=1, method="chatiot"):
    """
    Execute the workflow in topological order and save results to a JSON file after each task.
    
    Args:
        workflow (dict): The workflow built by build_workflow_from_search_space.
        problem (str): The problem to solve.
    
    Returns:
        dict: Final outputs of nodes with no outgoing edges for each task.
    """
    results = {}
    code_quality_all  = 0
    total_input_token_all = 0
    total_output_token_all = 0
    total_time_cost_all = 0
    final_outputs_all = []
    ...
    for i in range(run_iters):
        ...
        print(f"Iteration {i + 1}/{run_iters} for task: {code_name}")
        executed = set()
        ...
        def execute_node(node_name, input_data, problem):
            ...
            print(f"Running {node_name} with params: {node['params']}")
        ...
        starts = [name for name in workflow if not any(name in v["next"] for v in workflow.values())]
        ...
        # evaluate the quality of the final outputs
        code_quality = evaluate_code_quality(app_type, final_outputs)
        code_quality_all  += code_quality
        total_input_token_all += total_input_token
        total_output_token_all += total_output_token
        total_time_cost_all += total_time_cost
        final_outputs_all.append(final_outputs)
...
def run_predefined_workflow_config(filepath, benchmark_tasks, baseline="chatiot"):
    ...
    for code_name, problem in zip(data['code_name'], data["problem"]):
        ...
        results =  run_workflow(workflow, code_name, problem, 3, iter_num, baseline)
    ...
    for type_name, problem in zip(data['code_name'], data["problem"]):
        ...
        while iter_num <= max_iters:
            ...
            try:
                ...
                results = run_workflow(workflow, type_name, problem, 10, iter_num, method="meta_search")
...
def genetic_search_for_each(benchmark_tasks, max_iters=10):
    """
...
def main():
    ...
    run_predefined_workflow_config("/home/hao/code/iotpilot2.0/src/workflow_config.json", benchmark, "IoTPilot")
